...
public class EntryPurger {
  ...
  public void purge(Long feedId, Date retainAfterDate) {
    ...
    var feedEntries = feedEntryRepository.findErasableEntryIdsBySubscriptionIdAndCreatedAtEarlierThanRetainDate(
      feedId,
      retainAfterDate,
      pageRequest);
    ...
    while (!feedEntries.getContent().isEmpty()) {
      ...
      feedEntries = feedEntryRepository.findErasableEntryIdsBySubscriptionIdAndCreatedAtEarlierThanRetainDate(
        feedId,
        retainAfterDate,
        pageRequest);
    }
