...
public interface SubscriptionEntryRepository extends CrudRepository<SubscriptionEntry, Long>, SubscriptionEntryRepositoryCustom {
  ...
  @Query("select id from subscription_entry where subscription_id = :subscriptionId and created_at < :retainDate and (seen is true or excluded is true)")
  List<Long> findAllIdsBySubscriptionIdAndCreatedAtIsLowerThan(@Param("subscriptionId") Long id, @Param("retainDate") OffsetDateTime retainDate);
