...
describe('src/app/js/entry/entry.component.spec.js', () => {
  ...
  beforeEach(() => {
    ...
    entryTags = reactComponent('EntryTags')
  ...
  it('should propagate item to child components', () => {
    ...
    expect(entryActions.bindings).toContainObject({seen: item.seen})
    expect(entryTags.bindings).toContainObject({tags: scope.item.tag})
    expect(entryContent.bindings).toEqual({...item})
  })

  it('should show or hide entryTags component based on showMore flag', () => {
    entryActions.bindings.onToggleShowMore()
    expect(element.querySelector('react-component[name="EntryTags"]')).not.toBeNull()

    entryActions.bindings.onToggleShowMore()
    expect(element.querySelector('react-component[name="EntryTags"]')).toBeNull()
  })
  ...
  it('should update tag when entryTags component fired onSelect event', () => {
    entryActions.bindings.onToggleShowMore()
    entryTags.bindings.onChange('tag1')
